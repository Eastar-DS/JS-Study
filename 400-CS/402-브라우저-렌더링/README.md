...
파일:
- `step5.5-이벤트루프와-렌더링/README.md`
- `step5.5-이벤트루프와-렌더링/01메인-스레드와-렌더-기회.md`
- `step5.5-이벤트루프와-렌더링/02requestAnimationFrame.md`

핵심:
- 메인 스레드는 JS 실행과 렌더링을 번갈아 처리한다.
- 한 프레임 흐름 안에서 마이크로태스크와 렌더 기회가 어디 위치하는지 이해한다.
- `requestAnimationFrame`은 다음 프레임 직전에 실행되어 화면 갱신과 박자를 맞춘다.
- 이 챕터는 가상 DOM과 React Fiber로 넘어가는 다리 역할을 한다.
